...
void UartServo_Ctrl(uint8_t id, uint16_t value, uint16_t time)
{
    ...
    if (value > MAX_PULSE)
        ...
        value = MIN_PULSE;
    uint8_t pos_H = value & 0xff;;
    uint8_t pos_L = (value >> 8) & 0xff;
...
uint8_t time_H = 0x00;
  ...
  uint8_t vel_L = 0x00;
    uint8_t checknum = (~(s_id + len + cmd + addr +
                          pos_H + pos_L + time_H + time_L+vel_H+vel_L)) & 0xff;
    uint8_t data[] = {head1, head2, s_id, len, cmd, addr,
                      pos_H, pos_L, time_H, time_L,vel_H,vel_L, checknum};
    ...
    if (New_Frame)
    {
        ...
        if (checknum == Rx_Data[7])
        {
            ...
            if (osMutexAcquire(servoMutexHandle, 10) == osOK) {
                // 临界区：安全地写入 read_value
                servo_value = read_value;
